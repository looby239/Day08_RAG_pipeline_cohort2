"""Criminal Agent — AgentExecutor bridge between A2A SDK and LangGraph."""
...
from criminal_agent.graph import create_graph
...
class CriminalAgentExecutor(AgentExecutor):
    """Bridges A2A RequestContext to the Criminal LangGraph agent."""
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(
            "CriminalAgent executing | task=%s context=%s trace=%s depth=%d",
        ...
        try:
            ...
            await emit(
                "criminal",
                "completed",
                "Criminal Agent produced analysis",
                kind="output",
                detail=answer[:1600],
                trace_id=trace_id,
                telemetry_url=telemetry_url,
            ...
            logger.exception("CriminalAgent execution error: %s", exc)
            await updater.failed(
                updater.new_agent_message(
                    parts=[Part(root=TextPart(text=f"Criminal analysis failed: {exc}"))]
                )
            )
