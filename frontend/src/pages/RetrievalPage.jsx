...
import { Search, Info, List } from 'lucide-react';
import { retrieve } from '../config/api';
...
const RetrievalPage = () => {
    ...
    const handleSearch = async (e) => {
        ...
        setError('');
        try {
            const res = await retrieve({ query, method, topK, threshold });
        ...
        <div className="animate-fade-in">
            ...
            <div className="glass-panel" style={{ padding: '24px', marginBottom: '32px' }}>
                <form onSubmit={handleSearch} style={{ display: 'flex', gap: '16px', alignItems: 'flex-end' }}>
                    ...
                    <div className="input-group" style={{ flex: 0.5, marginBottom: 0 }}>
                        <label>Top-K</label>
                        <input
                            type="number"
                            className="text-input"
                    ...
                    <div className="input-group" style={{ flex: 0.6, marginBottom: 0 }}>
            ...
            {error && (
                <div className="glass-panel" style={{ padding: '16px', marginBottom: '24px', borderLeft: '4px solid var(--error)', color: 'var(--error)' }}>
                    Lỗi API: {error}
                </div>
            )}
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                        <span style={{ background: 'var(--accent-color)', color: 'white', padding: '4px 12px', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 'bold' }}>
                                            {r.metadata?.type || r.source || method}
                                        </span>
                                        <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Nguồn: {r.metadata?.source || 'unknown'}</span>
                                    </div>
