...
import { Send, Bot, User, FileText, ToggleLeft, ToggleRight, ChevronDown } from 'lucide-react';
...
const ChatPage = () => {
    ...
    const handleSend = async (e, textOverride = null) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'assistant', content: `Đã xảy ra lỗi khi gọi API: ${error.message}` }]);
        } finally {
            setIsLoading(false);
        }
    };
                ...
                {isOpen && (
                    <div style={{ padding: '14px', borderTop: '1px solid var(--panel-border)', background: '#FAFAFB', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {sources.map((src, idx) => {
                            ...
                            const scoreClass = score >= 0.8 ? '#31A24C' : (score >= 0.5 ? '#F5A623' : '#FA3E3E');
                            const scoreBg = score >= 0.8 ? '#E6F4EA' : (score >= 0.5 ? '#FEF6E9' : '#FCE8E8');
                            const scoreLbl = score >= 0.8 ? 'CAO' : (score >= 0.5 ? 'TB' : 'THẤP');
