    ...
    async def delegate_to_legal_agent(question: str) -> str:
        ...
        try:
            await emit(
                "customer",
                "running",
                "Calling service discovery",
                kind="tool",
                tool="registry.discover",
                tool_input={"task": "legal_question"},
                trace_id=trace_id,
            ...
            await emit(
                "customer",
                "completed",
                "Law Agent endpoint discovered",
                kind="tool",
                tool="registry.discover",
                tool_input={"task": "legal_question"},
                tool_output={"endpoint": endpoint},
                trace_id=trace_id,
                telemetry_url=telemetry_url,
            )
            await emit(
                "customer",
                "running",
                "Delegating question to Law Agent",
                kind="tool",
                tool="a2a.delegate",
                tool_input={"endpoint": endpoint, "question": question},
                trace_id=trace_id,
                telemetry_url=telemetry_url,
            )
            ...
            await emit(
                "customer",
                "completed",
                "Law Agent returned an A2A response",
                kind="tool",
                tool="a2a.delegate",
                tool_input={"endpoint": endpoint, "question": question},
